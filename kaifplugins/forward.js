...
module.exports = {
    ...
    kaif_handler: async (kaif_sock, kaif_origin, context) => {
        ...
        if (!quoted) {
            return await kaif_sock.sendMessage(kaif_origin, { text: '⚠️ Please reply to a message you want to forward.' });
        }
        ...
        if (!inputArgs) {
            ...
            return await kaif_sock.sendMessage(kaif_origin, { text: usage });
        }

        const targetJids = inputArgs.split(',').map(j => j.trim()).filter(j => j.length > 0);
        if (targetJids.length === 0) {
            return await kaif_sock.sendMessage(kaif_origin, { text: '⚠️ No valid JIDs found.' });
        }
