...
async function searchSsrmovies(query) {
    const searchUrl = 'https://ssrmovies.faith/?s=' + encodeURIComponent(query);
    ...
    for (const match of matches) {
        ...
        if (hrefM && titleM && hrefM[1].includes('ssrmovies.faith/')) {
            const url = hrefM[1];
...
async function sendMovieFile(kaif_sock, kaif_origin, selected, kaif_msg) {
    ...
    try {
        ...
        const remoteStream = await getRemoteStream(selected.downloadUrl);
...
module.exports = {
    ...
    kaif_handler: async (kaif_sock, kaif_origin, context) => {
        ...
        if (!query) {
            ...
            helpMsg += '• .movie https://ssrmovies.faith/movie-slug/ — View 1080p, 720p, 480p options\n';
        ...
        try {
            ...
            if (query.includes('ssrmovies.faith/')) {
