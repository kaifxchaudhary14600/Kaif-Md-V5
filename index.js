...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
// DEDUPLICATION & SEQUENTIAL AUTO-FORWARD QUEUE (Strict Order & 0% Media Corruption)
// -----------------------------------------------------------------------------
const forwardedMsgIds = new Set();

class SequentialForwardQueue {
    constructor() {
        this.queue = [];
...
const globalForwardQueue = new SequentialForwardQueue();
...
async function getCachedBotConfig(sessionId) {
    const now = Date.now();
    if (cachedBotConfig && (now - cachedBotConfigTime < 60 * 1000)) {
        return cachedBotConfig;
    }
    try {
        const cfg = await kaif_getBotConfig(sessionId);
        if (cfg) {
            cachedBotConfig = cfg;
            cachedBotConfigTime = now;
        }
        return cfg;
    } catch (e) {
        return cachedBotConfig;
...
async function getCachedGlobalAutoForward(sessionId) {
    const now = Date.now();
    if (cachedGlobalForward && (now - cachedGlobalForwardTime < 60 * 1000)) {
        return cachedGlobalForward;
    }
    try {
        const cfg = await kaif_getGlobalAutoForward(sessionId);
        if (cfg) {
            cachedGlobalForward = cfg;
            cachedGlobalForwardTime = now;
        }
        return cfg;
    } catch (e) {
        return cachedGlobalForward;
...
async function startSession(sessionId) {
    ...
    kaif_sock.ev.on('messages.upsert', async kaif_m => {
        ...
        for (const kaif_msg of kaif_m.messages) {
            ...
            // 2. Auto Crown Reaction (Non-blocking async fire-and-forget)
            ...
            // 1. GLOBAL AUTO FORWARD LOGIC (High speed RAM cache, Deduplication & Sequential Queue)
            if (kaif_origin.endsWith("@g.us") && !kaif_msg.key.fromMe) {
                try {
                    const msgId = kaif_msg.key?.id;
                    if (msgId && !forwardedMsgIds.has(msgId)) {
                        forwardedMsgIds.add(msgId);
                        if (forwardedMsgIds.size > 2000) {
                            const firstVal = forwardedMsgIds.values().next().value;
                            forwardedMsgIds.delete(firstVal);
                        }
                        ...
                        if (globalCfg?.enabled && globalCfg.sourceJids?.includes(kaif_origin) && globalCfg.targetJids?.length > 0) {
                            ...
                            const realMsg = unwrapMessage(kaif_msg.message);
                            const isText = !!(realMsg.conversation || realMsg.extendedTextMessage);
                            const isImage = !!realMsg.imageMessage;
                            const isVideo = !!realMsg.videoMessage;
                            const isAudio = !!realMsg.audioMessage;
                            const isDocument = !!realMsg.documentMessage;
                            const isSticker = !!realMsg.stickerMessage;

                            let shouldForward = true;
                            if (isText && globalCfg.forwardText === false) shouldForward = false;
                            else if (isImage && globalCfg.forwardImage === false) shouldForward = false;
                            else if (isVideo && globalCfg.forwardVideo === false) shouldForward = false;
                            else if (isAudio && globalCfg.forwardAudio === false) shouldForward = false;
                            else if (isDocument && globalCfg.forwardDocument === false) shouldForward = false;
                            else if (isSticker && globalCfg.forwardSticker === false) shouldForward = false;
                            ...
                            if (shouldForward) {
                                ...
                                globalForwardQueue.enqueue(async () => {
